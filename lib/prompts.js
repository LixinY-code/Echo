...
- mirror.signals：2-4 个情绪关键词（如"焦虑""自我怀疑""压力"）
- mirror.strategy：本次回应的策略说明，用自然的中文解释为什么这样回（如"用温和的挑战帮你松动绝对化想法，而不是直接给答案"）
- mirror.blindspots：1-3 条 AI 可能忽略的视角，每条是一句完整的话（如"你今天的疲惫程度、这件事具体的前因后果、你曾尝试过的解决办法"），前端会连成段落展示
