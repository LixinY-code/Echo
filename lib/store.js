...
async function getProfile(userId) {
  ...
  const { data, error } = await supabase
    .from('user_profiles')
    .select(
      'nickname, personality, tags, onboarded, known_topics, last_emotion, detected_scenario, interaction_count, profile_insights',
    )
    .eq('user_id', userId)
    .maybeSingle()
  if (error) {
...
/**
 * 每次对话后更新用户侧写（merge 增量）
 * @param update 来自 DeepSeek 的 profile_update：{ detected_scenario, new_topics, emotion_signal, insight }
 */
