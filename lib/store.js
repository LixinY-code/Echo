...
async function getProfile(userId) {
  ...
  const { data, error } = await supabase
    .from('user_profiles')
    .select(
      'nickname, personality, tags, onboarded, known_topics, last_emotion, detected_scenario, interaction_count, profile_insights, emotion_history, pattern_hints',
    )
    .eq('user_id', userId)
    .maybeSingle()
  if (error) {
 ...
 * @param update 来自 DeepSeek 的 profile_update：{ detected_scenario, new_topics, emotion_signal, emotion_reason, insight, pattern_hint }
 */
