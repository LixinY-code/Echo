...
const { CHAT_PROMPT, LAB_PROMPT, SUMMARY_PROMPT } = require('./prompts')
...
async function generateSummary(chatText) {
  ...
  const raw = completion.choices[0]?.message?.content || '{}'
  const parsed = JSON.parse(raw)
  return parsed.summary || '一次温暖的对话。'
}

module.exports = { generateReply, generateLabVersions, generateSummary }
